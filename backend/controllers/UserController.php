...
class UserController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($request->isAjax) {
            ...
            if ($request->isGet) {
                ...
                // Set password hash
                $password = $request->post('User')['password'] ?? null;
                if (!empty($password)) {
                    $model->setPassword($password);
                    $model->generateAuthKey();
                } else {
                    // If no password provided, we need to generate one (or fail)
                    // For security, we'll require password on create.
                    return [
                        'title' => Yii::t('yii2-ajaxcrud', 'Create New') . " User",
                        'content' => $this->renderAjax('create', [
                            'model' => $model,
            ...
            // Non-AJAX request (not used in your current setup)
    ...
    public function actionUpdate($id)
    {
        ...
        $roleList = $this->getRoleList();
        $auth = Yii::$app->authManager;
        $currentRole = $auth->getRolesByUser($id);
        $selectedRole = !empty($currentRole) ? reset($currentRole)->name : null;
        ...
        if ($request->isAjax) {
            ...
            if ($request->isGet) {
                ...
                if (!empty($password)) {
                    ...
                    // Unset password attribute so it won't be validated (if validation rule exists)
                    $model->password = null;
                }
                ...
                if ($model->save()) {
                    ...
                    return [
                        ...
                        'content' => $this->renderAjax('update', [
                            'model' => $model,
                            'roleList' => $roleList,
                            'selectedRole' => $request->post('role') ?? $selectedRole,
                        ]),
                        'footer' => Html::button(Yii::t('yii2-ajaxcrud', 'Close'), ['class' => 'btn btn-default pull-left', 'data-bs-dismiss' => 'modal']) .
                            Html::button(Yii::t('yii2-ajaxcrud', 'Save'), ['class' => 'btn btn-primary', 'type' => 'submit'])
                    ];
